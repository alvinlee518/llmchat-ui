<template>
  <div>
    ...
    <MenuScopeModal ref="menuScopeModalRef" />
    <DataScopeModal ref="dataScopeModalRef" @on-close="onSearch" />
    <UserScopeModal ref="userScopeModalRef" />
  </div>
</template>

<script setup lang="ts">
  import { h, onMounted, reactive, ref } from 'vue';
  import { NText, NButton, NDivider, NPopconfirm, NDropdown } from 'naive-ui';
  import { RowStateOptions, findDataScopeLabel, findRowStateLabel } from '@/utils/optionsUtil';
  import { queryPage, remove } from '@/api/system/role';
  import CreateModal from './component/CreateModal.vue';
  import MenuScopeModal from './component/MenuScopeModal.vue';
  import DataScopeModal from './component/DataScopeModal.vue';
  import UserScopeModal from './component/UserScopeModal.vue';
  const createModalRef: any = ref(null);
  const menuScopeModalRef: any = ref(null);
  const dataScopeModalRef: any = ref(null);
  const userScopeModalRef: any = ref(null);
    ...
    },
    {
      ...
      render(row) {
        return h(
          ...
          {},
          {
            default: () => findDataScopeLabel(row.dataScope),
          }
        );
      },
    },
          ...
          h(
            NDropdown,
            {
              ...
              onSelect: (key: string) => {
                switch (key) {
                  case 'menuScope':
                    menuScopeModalRef.value.openModal(row.id);
                    break;
                  case 'userScope':
                    userScopeModalRef.value.openModal(row.id);
                    break;
                  case 'dataScope':
                    dataScopeModalRef.value.openModal(row.id);
                    break;
                }
              },
            },
