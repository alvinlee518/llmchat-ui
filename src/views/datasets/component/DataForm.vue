<template>
  ...
  >
    <n-form-item label="数据类型" path="dataType">
      <n-radio-group
        @change="(value) => (form_data.fileList = [])"
        style="width: 100%"
      ...
      >
        <n-flex align="center" :wrap="false" class="n-card n-card--bordered p-3 mb-2 h-24">
          ...
          <div>
            <n-flex align="center">
              <n-icon :size="24"> <FileWordFilled /> </n-icon> 非结构化数据
            </n-flex>
        ...
        <n-flex align="center" :wrap="false" class="n-card n-card--bordered p-3 h-24">
          ...
          <div>
            <n-flex align="center">
              <n-icon :size="24"> <FileExcelFilled /> </n-icon> 结构化数据
            </n-flex>
    ...
    <n-form-item label="上传文档" path="fileList">
      ...
      >
        <n-upload-dragger>
          ...
          <n-p depth="3" style="font-size: 12px">
            当前支持 XLSX / XLS / CSV 格式的文档每次最多上传10个文件，每个文件不超过50MB
          </n-p>
        </n-upload-dragger>
        <n-p>
      ...
      >
        <n-upload-dragger>
          ...
          <n-p depth="3" style="font-size: 12px">
            支持格式：TXT、Markdown、PDF、DOC、HTML每次最多上传10个文件，每个文件不超过50MB，建议上传前规范文件的分段标识
          </n-p>
        </n-upload-dragger>
      </n-upload>
    </n-form-item>
