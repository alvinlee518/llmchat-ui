<template>
  ...
  >
    ...
    <div v-else>
      <n-form-item label="分段规则" path="chunkType">
        <n-radio-group style="width: 100%" v-model:value="form_data.chunkType" name="chunkType">
      ...
      <n-form-item v-if="form_data.chunkType === 1" label="分段标识符" path="seperator">
        <n-select v-model:value="form_data.seperator" :options="splitor_options" :multiple="true" />
      </n-form-item>
      <n-form-item v-if="form_data.chunkType === 1" label="分段预估长度" path="chunkSize">
        <n-input-number v-model:value="form_data.chunkSize" :min="1" :max="2048" />
      </n-form-item>
      <n-form-item v-if="form_data.chunkType === 1" label="分段重叠长度" path="chunkOverlap">
        <n-input-number v-model:value="form_data.chunkOverlap" :min="0" :max="512" />
      </n-form-item>
      <n-form-item label="数据清洗" path="dataProcess">
        <n-checkbox-group v-model:value="form_data.dataProcess">
          <n-space vertical>
